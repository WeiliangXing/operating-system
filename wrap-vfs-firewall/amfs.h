...
/*maximum pages read for pattern in pattern files*/
#define MAX_PATTERN_PAGES 10
...
/* pattern structure to store info about the pattern files*/
struct patterns{
	int err;
	int pats_len;
	char *db_dir;
...
struct amfs_sb_info {
	...
	struct patterns pattdb;
};

/*pattern db file operations*/
extern void amfs_sort_pat(struct patterns *patptr);
extern void amfs_swap_pat(char **a, char **b);
extern char **amfs_list_pat(struct patterns *patptr);
extern void amfs_remove_dup_pat(struct patterns *patptr);
extern int amfs_search_pat(struct patterns *patptr, char *str);
extern int amfs_remove_pat(struct patterns *patptr, char *str);
extern int amfs_add_pat(struct patterns *patptr, char *str);
extern int write_pattern_file(struct patterns *patptr, char *dir);
extern int amfs_strcmp(struct patterns *patptr, char *str);
extern int amfs_read_file_strcmp(struct patterns *patptr, struct file *file);
extern int amfs_crypt(struct patterns *patptr, int mode);

extern int amfs_gen_key(struct patterns *patptr, char *pwd);
